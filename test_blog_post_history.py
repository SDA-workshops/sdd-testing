...
class TestBlogPostHistory(unittest.TestCase):
    ...
    def test_change_description_should_change_description(self):
        blogpost = BlogPostHistory("Example", "...")
        expacted_description = "Some description"
        blogpost.save = Mock()

        blogpost.change_description(expacted_description)

        self.assertEqual(blogpost.description, expacted_description)
        blogpost.save.assert_called_once_with()

    def test_change_tittle_should_raise_on_os_error(self):
        blogpost = BlogPostHistory("Example","...")
        blogpost.save = Mock(side_effect=OSError)

        self.assertRaises(
            Exception, blogpost.change_title, "New tittle")
